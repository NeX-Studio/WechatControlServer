...
database = sqlite3.connect(PATH+"/../log/appid.db") # Initializing database
...
class token_acquirer:
    ...
    def __init__(self):
        ...
        # Initialize database
        # Class attribute?
        token_acquirer.db = web.database(dbn="sqlite",db=PATH+r"\..\log\appid.db")
        self.last_time = 0
        msg = time.strftime("%Y-%m-%d %H:%M:%S")+"\tProcessing request..."
        print(msg)
        with open(PATH+"/../log/request_log","a") as log:
            log.writelines(msg)
            log.write("\n")
            log.flush()
            log.close()
    ...
    def get_token(self,credential):
        with open(PATH+r"\..\log\request_log","a") as log:
